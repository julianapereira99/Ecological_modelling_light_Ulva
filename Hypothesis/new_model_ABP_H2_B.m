...
%==================================
 ...
 inputs.PEsol.id_global_theta=char('KA','aAB','aBP','aAP','aPB' ); 


                            
inputs.PEsol.global_theta_min=[ 6     -1e-5    -1e-6 -1e-6 -1e-6    ] ;
inputs.PEsol.global_theta_max=[ 8.25  1e-6      1e-6    1e-4 1e-6   ] ;
inputs.PEsol.global_theta_guess=[ results.fit.thetabest(1:5)' ];

j=5;
for iexp=1:inputs.exps.n_exp
    inputs.PEsol.id_local_theta{iexp}=char('muA','aBA');
    inputs.PEsol.local_theta_min{iexp}=[  1e-5 -1e-6]; 
    inputs.PEsol.local_theta_max{iexp}=[  0.075 1e-6] ; 
   inputs.PEsol.local_theta_guess{iexp}=[results.fit.thetabest(j+1:j+2)'];
...
%Note that algae do not grow in darkness therefore mu=0 
    ...
    inputs.PEsol.local_theta_min{4}=[  0    -1e-7]; 
    inputs.PEsol.local_theta_max{4}=[ 1e-7 1e-7]; 
