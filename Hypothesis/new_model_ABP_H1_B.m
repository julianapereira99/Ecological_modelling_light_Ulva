...
inputs.pathd.runident='H1_muAlight_DEF';               % [] Identifier required in order not to overwrite previous results
...
%==================================
 ...
 inputs.PEsol.id_global_theta=char('KA','aBP','aPB','aAB' ); 


                            
inputs.PEsol.global_theta_min=[ 6     -1e-7    -1e-8 -1e-7     ] ;
inputs.PEsol.global_theta_max=[ 8.5  1e-6    1e-6   1e-6     ] ;
inputs.PEsol.global_theta_guess=[ results.fit.thetabest(1:4)' ];

j=4;
for iexp=1:inputs.exps.n_exp
    inputs.PEsol.id_local_theta{iexp}=char('muA','aBA','aPA');
    inputs.PEsol.local_theta_min{iexp}=[  1e-5 -1e-6 -1e-6]; 
    inputs.PEsol.local_theta_max{iexp}=[  0.1 1e-6 1e-6] ; 
   inputs.PEsol.local_theta_guess{iexp}=[results.fit.thetabest(j+1:j+3)'];
...
%Note that algae do not grow in darkness therefore mu=0 
    ...
    inputs.PEsol.local_theta_min{4}=[  0  -1e-8 -1e-8 ]; 
    inputs.PEsol.local_theta_max{4}=[ 1e-5 1e-8 1e-8];
